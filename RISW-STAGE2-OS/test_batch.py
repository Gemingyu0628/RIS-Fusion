...
modelname = "No14_moreepoch_416_60"
device = 'cuda' if torch.cuda.is_available() else 'cpu'
...
for dataset_name in ["LLVIP_TEST","TNO","MSRS_test","M3FD_test"]:#"TNO","MSRS_test","M3FD_test"
    print("\n"*2+"="*80)
    print("The test result of "+dataset_name+' :')
    test_folder=os.path.join('test_img',dataset_name) 
    test_out_folder=os.path.join('test_results',dataset_name)
    test_out_folder=os.path.join(test_out_folder,modelname)
    if os.path.exists(test_out_folder) == False:
        os.mkdir(test_out_folder)

    with torch.no_grad():
        for img_name in os.listdir(os.path.join(test_folder,"ir")):
            
            imageA = imread(os.path.join(test_folder,"ir",img_name)).astype(np.float32)[None,None, :, :]
            if(len(imageA.shape)==5):
                imageA = image_read_cv2(os.path.join(test_folder,"ir",img_name), mode='RGB').transpose(2,0,1)[np.newaxis,0:1, ...]
            imageB = image_read_cv2(os.path.join(test_folder,"vi",img_name), mode='RGB').transpose(2,0,1)[np.newaxis, ...]
            
            name=img_name[:-4]
            # Fusion = image_read_cv2(os.path.join(test_folder,"fusion",str(name)+'.bmp'), mode='RGB').transpose(2,0,1)[np.newaxis, ...]/255.0
            
            # imageA, imageB = torch.FloatTensor(imageA),torch.FloatTensor(imageB)
            ...
            #save_image(pre_fuse, test_out_folder+'/prefuse_'+name+'.bmp')
            save_image(Fused_final, test_out_folder+'/'+name+'.bmp')
            #save_image(data_common,test_out_folder+'/'+name+'_3.png',nrow=1, normalize=True,  range=(0, 1), cmap='gray')
            #save_image(data_1,test_out_folder+'/'+name+'_4.png',nrow=1, normalize=True,   range=(0, 1),cmap='gray')
            #save_image(data_2,test_out_folder+'/'+name+'_5.png',nrow=1, normalize=True, range=(0, 1), cmap='gray')
            
            #save_image(fuse,test_out_folder+'/'+name+'_6.png')
            
            save_image(seg,test_out_folder+'/mask_'+name+'.bmp')
