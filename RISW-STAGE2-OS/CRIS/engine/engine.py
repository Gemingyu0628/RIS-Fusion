...
from CRIS.utils.misc import (AverageMeter, ProgressMeter, concat_all_gather,
                        trainMetricGPU)
import torchvision
...
def train(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, (image, image2, target, text, fusion) in enumerate(train_loader):
        ...
        # forward
        with amp.autocast():
            pred, target, loss = model(fusion, text, target)
