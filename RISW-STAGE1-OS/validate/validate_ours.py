...
def validate(args, data_loader, model, local_rank=0, visualize=False, logger=None, save_cam=False):
    ...
    for idx,(samples, targets) in enumerate(data_loader):
        ...
        img = samples['img_ir'].cuda(local_rank,non_blocking=True) # [B,3,H,W]
        img2 = samples['img_vis'].cuda(local_rank,non_blocking=True) # [B,3,H,W]
        batch_size = img.size(0)
        target = targets['target'].cuda(local_rank,non_blocking=True)[:,1,...] #[B,ori_H,ori_W]
        ...
        bbox = generate_bbox(target.cpu().numpy().astype(np.float64))[0]
        bbox = torch.tensor(bbox[0:4]).unsqueeze(0)
