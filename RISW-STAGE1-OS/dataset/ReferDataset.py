...
class ReferDataset(data.Dataset):
    def __init__(self,
        ...
        for index, r in enumerate(self.ref_ids):
            ...
            # for each sentence of each image
            for i,(el,sent_id) in enumerate(zip(ref['sentences'],ref['sent_ids'])):
                sentence_raw = el['sent']
                word_id = self.tokenizer(sentence_raw).squeeze(0)[:self.max_tokens] #从1,77 到 20
                word_id = np.array(word_id)
                word_mask = np.array(word_id>0,dtype=int)

                sentences_for_ref.append(torch.tensor(word_id).unsqueeze(0)) # 20 到 1,20
