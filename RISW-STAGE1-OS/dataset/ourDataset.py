...
class CustomDataset(Dataset):
    def __init__(self, ir_dir, vis_dir, text_dir, image_transforms=None, max_tokens=20, bert_tokenizer='clip',):
        """
        ir_dir: 红外图像的文件夹路径
        vis_dir: 可见光图像的文件夹路径
        text_dir: 文本描述文件夹路径
    ...
    def _load_text(self, text_id, idx):
        """加载对应的文本描述"""
        text_files = [f"{idx+1}_1.txt", f"{idx+1}_2.txt", f"{idx+1}_3.txt"]
        positive_text = os.path.join(self.text_dir, text_files[0])
        negative_texts = [os.path.join(self.text_dir, text_files[1]), os.path.join(self.text_dir, text_files[2])]
        
        with open(positive_text, "r") as f:
            positive_desc = f.read()
        
        negative_descs = []
        for neg_file in negative_texts:
            with open(neg_file, "r") as f:
                negative_descs.append(f.read())
        
        return positive_desc, negative_descs
    ...
    def __getitem__(self, idx):
        ...
        # 加载对应的文本描述
        positive_text, negative_texts = self._load_text(self.text_dir, idx)

        word_id_pos = self.tokenizer(positive_text).squeeze(0)[:self.max_tokens] #从1,77 到 20
        word_id_pos = np.array(word_id_pos)

        word_id_ne1 = self.tokenizer(negative_texts[0]).squeeze(0)[:self.max_tokens]
        word_id_ne1 = np.array(word_id_ne1)

        word_id_ne2  = self.tokenizer(negative_texts[1]).squeeze(0)[:self.max_tokens]
        word_id_ne2 = np.array(word_id_ne2)
        
        samples = {
            "img_ir": ir_image,
            "img_vis": vis_image,
            "word_ids": word_id_pos,
            "neg_word_ids": torch.stack((torch.tensor(word_id_ne1),torch.tensor(word_id_ne2))),
        }
        
        targets = {
            "target": ir_image.unsqueeze(0),
            "sentences": positive_text,
        }

        return samples, targets,image_path_1,image_path_2
